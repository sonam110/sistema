...
class ReportController extends Controller
{
	...
	public function salesReportList(Request $request) {
        ...
        if($whereRaw != '') {
            ...
            $query = booking::select('id','created_by','firstname','lastname','tranjectionid','payableAmount','paymentThrough','deliveryStatus','created_at')->where('created_by', '!=', null)->orderBy('id','DESC')
            ->with('createdBy')
            ->whereRaw($whereRaw)
            ->get();
        } else {
    ...
    public function downloadsalesReport(Request $request) {
	    $fileName = 'sales_'.rand(0,999).time().'.csv';
    ...
    public function downloadpurchaseReport(Request $request) {
	    $fileName = 'purchase_'.rand(0,999).time().'.csv';
